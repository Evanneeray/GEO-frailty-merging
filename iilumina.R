...
###### extracting raw data from individual GSEs #########
...
write.csv(GSE101710_fixed,'./fixed_data_sets/GSE101710_all_data.txt',quote = F,row.names = F)
...
write.csv(GSE65218_fixed,'./fixed_data_sets/GSE65218_all_data.txt',quote = F,row.names = F)

#there is no platform file available
#the probe IDs are very strange
###############################
rm(list=ls())
...
write.csv(GSE65219_fixed,'./fixed_data_sets/GSE65219_all_data.txt',quote = F,row.names = F)
...
write.csv(GSE59635_fixed,'./fixed_data_sets/GSE59635_all_data.txt',quote = F,row.names = F)
...
write.csv(GSE59654_fixed,'./fixed_data_sets/GSE59654_all_data.txt',quote = F,row.names = F)
...
write.csv(GSE94497_fixed,'./fixed_data_sets/GSE94497_all_data.txt',quote = F,row.names = F)
...
write.csv(GSE94496_fixed,'./fixed_data_sets/GSE94496_all_data.txt',quote = F,row.names = F)
###############################
rm(list=ls())
...
#Combining GSMs into one bif data frame
files<-list.files(pattern = 'GSM')
for(file in files) {assign((str_sub(file, 1, 10)),read.csv(file,sep='\t'))}
GSE129534_data <- bind_cols(mget(ls(pattern = "GSM"))) %>% 
  select(-matches("symbol\\d+"))
names<-ls(pattern = "GSM")
colnames(GSE129534_data)[2:17]<-names
write.csv(GSE129534_data,'GSE129534_combined.csv',quote = F,row.names = F)

#saving a copy in forlder with all the fixed data
GSE129534_data<-read.csv('./GSE129534/GSE129534_combined.csv')
